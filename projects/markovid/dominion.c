...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  {
  case adventurer:
	  return playAdventurer(drawntreasure, currentPlayer, state, cardDrawn, temphand, z);
    ...
    case outpost:
		return playOutpost(state, handPos, currentPlayer);
...
int playAdventurer(int drawnTreasure, int currentPlayer, struct gameState* state, int cardDrawn, int* tempHand, int tempHandIndex)
{
	while (drawnTreasure < 2)
	{
		...
		{
			tempHand[tempHandIndex] = cardDrawn;
			state->handCount[currentPlayer]--;
			tempHandIndex++;
		}
	}
...
int playSmithy(int* currentPlayer, struct gameState* state, int* handPosition)
{
	drawCard(currentPlayer, state);
	drawCard(currentPlayer, state);
	drawCard(currentPlayer, state);
...
int playOutpost(struct gameState* state, int handPosition, int currentPlayer)
{
